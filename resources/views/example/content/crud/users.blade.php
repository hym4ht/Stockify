                        ...
                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                            ...
                            <td class="p-4 text-base font-normal text-gray-900 whitespace-nowrap dark:text-white">
                                <div class="flex items-center">
                                    ...
                                    Staff Gudang
                                    @elseif($user->hasRole('Manajer Gudang'))
                            ...
                            <td class="p-4 space-x-2 whitespace-nowrap">
                                @if(auth()->user()->hasRole('Admin') && $user->hasRole('unconfirmed'))
                                <form action="{{ route('users.confirmRole', $user->id) }}" method="POST" class="inline-flex items-center space-x-2">
                                    @csrf
...
<tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
    ...
    <td class="flex items-center p-4 mr-12 space-x-6 whitespace-nowrap">
        <img class="w-10 h-10 rounded-full" src="{{ asset('static/images/users/default-avatar.png') }}" alt="avatar">
        <div class="text-sm font-normal text-gray-500 dark:text-gray-400">
            <div class="text-base font-semibold text-gray-900 dark:text-white">{{ $user->name }}</div>
            <div class="text-sm font-normal text-gray-500 dark:text-gray-400">{{ $user->email }}</div>
        </div>
    ...
    <td class="p-4 space-x-2 whitespace-nowrap">
        @if(auth()->user()->hasRole('Admin') && $user->hasRole('unconfirmed'))
        <form action="{{ route('users.confirmRole', $user->id) }}" method="POST" class="inline">
            @csrf
            @method('PUT')
            <button type="submit" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-800">
                Confirm Role
            </button>
        </form>
        @endif
        <a href="{{ route('users.edit', $user->id) }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
            Edit user
        </a>
